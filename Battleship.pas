...
procedure display(var localGrid : Grid);
var i : integer;
    c : char;
begin
	writeln;
	...
	writeln('|');
    for i := 1 to 44 do write('-');
	writeln;
	
    for c := 'A' to 'J' do
    begin
		write(' ', c, ' ');
		for i := 0 to 9 do write('| ', localGrid[i,c].state,' ');
		writeln('|');
		for i := 1 to 44 do write('-');
		writeln;
    end;
...
var strMain : string;
    ...
    i, j, tmpInt : integer;
    c, tmpChr : char;
    isBoatSizeWrong, isBoatOverriding : boolean;
...
begin
    ...
    repeat
        ...
        repeat
			...
			if ((pos1.posY <> pos2.posY) or (abs(pos2.posX - pos1.posX) <> boatSize-1)) and ((pos1.posX <> pos2.posX) or (abs(ord(pos2.posY) - ord(pos1.posY)) <> boatSize-1)) then
			begin
				isBoatSizeWrong := true;
				writeln;
				writeln('Wrong size of boat !');
			end;
			writeln;
        until isBoatSizeWrong = false;
